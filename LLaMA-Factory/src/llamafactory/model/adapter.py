...
def _setup_lora_tuning(
    ...
    # 冻结所有包含lora_B的参数
    for name, param in model.named_parameters():
        if "lora_BB" in name and is_trainable:
            param.requires_grad = False
            logger.info_rank0(f"冻结参数: {name}")

    return model

def _setup_prefix_tuning(
    config: "PretrainedConfig",
    model: "PreTrainedModel",
    model_args: "ModelArguments",
    finetuning_args: "FinetuningArguments",
    ...
    if is_trainable:
        logger.info_rank0("Fine-tuning method: {}".format("Prefix-Tuning"))

    adapter_to_resume = None
    from peft import PrefixTuningConfig, TaskType, PeftType
...
def _setup_prompt_tuning(
    ...
    if is_trainable:
        logger.info_rank0("Fine-tuning method: {}".format("Prompt-Tuning"))
    ...
    if is_trainable:    
        ...
        peft_config = PromptTuningConfig(
            ...
            # prompt_tuning_init_text="Please output an entailment or not_entailment according to the contents of sentence1 and sentence2 determine whether the two sentences have an entailment relationship.", #RTE
            # prompt_tuning_init_text="Please select the correct answer from the choices based on the content of the question.", # MMLU
            # prompt_tuning_init_text="Please judge whether the sentence is positive or negative based on the content of the user review.", #IMDB
            # prompt_tuning_init_text="Please judge the correctness or error of the sentence syntax according to the input sentence content output right or error.", # COLA
            prompt_head = 4,
...
def _setup_p_tuning(
    ...
    if is_trainable:
        logger.info_rank0("Fine-tuning method: {}".format("P-Tuning"))

    adapter_to_resume = None
    from peft import PromptEncoderConfig, TaskType, PromptEncoderReparameterizationType
...
def init_adapter(
    ...
    if finetuning_args.finetuning_type == "full":
        ...
        model = _setup_prefix_tuning(config, model, model_args, finetuning_args, is_trainable, cast_trainable_params_to_fp32)
    elif finetuning_args.finetuning_type == "prompt":
        model = _setup_prompt_tuning(config, model, model_args, finetuning_args, is_trainable, cast_trainable_params_to_fp32)
    elif finetuning_args.finetuning_type == "ptuning":
        model = _setup_p_tuning(config, model, model_args, finetuning_args, is_trainable, cast_trainable_params_to_fp32)
    else:
